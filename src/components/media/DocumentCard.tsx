    ...
    <div className="bg-white rounded-2xl p-6 shadow-[var(--shadow-card)] flex gap-4 items-start group hover:shadow-[var(--shadow-card-hover)] transition-shadow duration-300">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2 mb-1">
          <h3 className="font-semibold text-neutral-800 text-sm">{doc.title}</h3>
          <span className="text-xs text-neutral-400 shrink-0">
            {doc.fileType}
            {doc.fileSize ? ` · ${doc.fileSize}` : ""}
          </span>
        </div>
        <p className="text-xs text-neutral-500 leading-relaxed mb-3">{doc.description}</p>
        <a
          href={doc.href}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-1.5 text-xs font-semibold text-brand-blue hover:text-brand-gold transition-colors"
        >
